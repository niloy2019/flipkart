...
<!-----------      JAVA CODE TO FETCH THE SEARCH PRODUCTS FROM DATABASE         --------------->	
	...
	<%
    	...
    	List<ProductDetails> list2=new ArrayList<ProductDetails>();
		...
		//Getting the Searching Product which user have entered in Search Box
		String search=request.getParameter("search").toLowerCase();
		
		//Code to Search The Products from Database
		for(int i=0;i<list.size();i++){
			ProductDetails s1=list.get(i);
			if(s1.getCategory()!=null){
				
					String categoery=s1.getCategory().toLowerCase();
					
					if(categoery.contains(search) || search.contains(categoery)){
						list2.add(s1);
					}
			}
		}
		...
		for(int i=0;i<list2.size();i++){
			...
			ProductDetails s1=list2.get(i);
			ProductDetails s2=list2.get(++i);
			ProductDetails s3=list2.get(++i);
			ProductDetails s4=list2.get(++i);
			ProductDetails s5=list2.get(++i);
...
<!-----------------------       HTML CODE FOR 1 ROW OF PRODUCTS          -------------------------------------->	
	...
	<div id="navbar"  style="background-color: white;margin:20px 0px 50px 0px;padding-bottom: 30px;">
...
<% if(list2.size()==0){%>
	...
	<div style="text-align: center;padding: 10px">
		<img alt="" src="images/noResultsFound.PNG">
	</div>
